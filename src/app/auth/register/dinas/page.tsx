...
import {
  ...
  CloudUpload, ChevronRight, AlertCircle,
} from "lucide-react";
...
export default function RegisterDinasPage() {
  ...
  const [form, setForm] = useState({
    namaPenanggung: "", jabatan: "", email: "",
    password: "", konfirmasi: "",
    jenisInstansi: "", namaInstansi: "", wilayahKerja: "",
  });
    ...
    <AuthLayout showAbstractBackground={true}>
      <div className="w-full max-w-[640px] bg-white border border-zinc-100 rounded-3xl p-8 md:p-10 shadow-sm">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Daftar sebagai Dinas Pendidikan</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Nama penanggung jawab</label>
            <input type="text" name="namaPenanggung" value={form.namaPenanggung} onChange={handleChange} placeholder="Sesuai KTP" required className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-[#00473e]/20 focus:border-[#00473e]/40" />
          </div>

          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Jabatan</label>
            <input type="text" name="jabatan" value={form.jabatan} onChange={handleChange} placeholder="mis. Kepala Bidang Pendidikan Dasar" required className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-[#00473e]/20 focus:border-[#00473e]/40" />
          </div>

          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Email dinas/institusi</label>
            <div className="relative">
              <Mail className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
              <input type="email" name="email" value={form.email} onChange={handleChange} placeholder="nama@dindik.go.id" required className="w-full border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-sm text-slate-700 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-[#00473e]/20 focus:border-[#00473e]/40" />
            </div>
          </div>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Jenis instansi</label>
            <div className="relative">
              <select name="jenisInstansi" value={form.jenisInstansi} onChange={handleChange} required className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 appearance-none outline-none focus:ring-2 focus:ring-[#00473e]/20 focus:border-[#00473e]/40 bg-white cursor-pointer pr-8">
                {jenisInstansiOptions.map((o) => (<option key={o.value} value={o.value} disabled={o.value === ""}>{o.label}</option>))}
              </select>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Nama instansi</label>
            <input type="text" name="namaInstansi" value={form.namaInstansi} onChange={handleChange} placeholder="mis. Dinas Pendidikan Kota Serang" required className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-[#00473e]/20 focus:border-[#00473e]/40" />
          </div>

          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Wilayah kerja</label>
            <div className="relative">
              <select name="wilayahKerja" value={form.wilayahKerja} onChange={handleChange} required className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 appearance-none outline-none focus:ring-2 focus:ring-[#00473e]/20 focus:border-[#00473e]/40 bg-white cursor-pointer pr-8">
                {wilayahOptions.map((o) => (<option key={o.value} value={o.value} disabled={o.value === ""}>{o.label}</option>))}
              </select>
              <span className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none text-xs">▾</span>
            </div>
          </div>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Surat penugasan resmi</label>
            <button type="button" onClick={() => fileRef.current?.click()} className="w-full border-2 border-dashed border-slate-200 rounded-2xl py-8 flex flex-col items-center gap-2 hover:border-[#00473e]/40 hover:bg-[#e6f4f1]/20 transition-all cursor-pointer">
              ...
              <span className="text-sm font-semibold text-[#00473e]">{fileName ?? "Unggah SK penugasan atau surat resmi instansi"}</span>
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <span className="text-sm text-slate-500 leading-relaxed">Saya menyatakan data yang diisi benar dan mewakili instansi resmi</span>
          </label>
